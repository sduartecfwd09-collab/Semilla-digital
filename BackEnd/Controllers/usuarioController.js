...
const changeStatus = async (req, res) => {
  try {
    ...
    if (!status) {
      return res.status(400).json({ success: false, message: 'El status es requerido' });
    }
...
const login = async (req, res) => {
  try {
    ...
    if (error.message.includes('Credenciales') || error.message.includes('no encontrad') || error.message.includes('inválid')) {
...
module.exports = { getAll, getById, create, update, remove, changeStatus, login, register, getProfile };
