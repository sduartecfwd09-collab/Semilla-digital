...
Definidos a nivel de **colección** (se ejecutan tras *cada* request):
...
| `adminToken` / `productorToken` / `usuarioToken` | *(vacío, se llena en runtime)* | secret |
| `adminId` / `productorId` / `usuarioId` | *(vacío)* | default |
| `feriaId`, `createdFeriaId`, `productoId`, `createdProductoId`, `recetaId`, `createdRecetaId`, `provinciaId`, `createdMensajeId`, `createdSolicitudId` | *(vacío)* | default |
