...
const Navbar: React.FC = () => {
  ...
  const showProfileDot = hasProfileNotif && notifEnabled && user?.role !== 'Administrador'
  const showContactDot = hasContactNotif && notifEnabled && user?.role !== 'Administrador'

  return (
    <nav className="pn-navbar">
      <Link to="/" className="pn-navbar-logo" onClick={handleScrollToTop}>
        <span className="pn-navbar-logo-agro">Agro</span><span className="pn-navbar-logo-map">Map</span>
      </Link>
      ...
      <ul className={`pn-navbar-links ${menuOpen ? 'open' : ''}`}>
        ...
        <li>
          <Link to="/comparar" className={`pn-navbar-link ${isActive('/comparar') ? 'active' : ''}`}>
            Productos
          </Link>
        </li>
        <li>
          <Link to="/recetas" className={`pn-navbar-link ${isActive('/recetas') ? 'active' : ''}`}>
            Recetas
          </Link>
        </li>
        <li>
          <Link to="/contacto" className={`pn-navbar-link ${isActive('/contacto') ? 'active' : ''}`}>
            Contáctanos
        ...
        {user?.role === 'Administrador' && (
          <li>
            <Link to="/admin" className={`pn-navbar-link ${location.pathname.startsWith('/admin') ? 'active' : ''}`}>
              Panel Admin
            </Link>
          </li>
        )}
        ...
        {user?.role === 'Productor' && (
          <li>
            <Link to="/productor" className={`pn-navbar-link ${location.pathname.startsWith('/productor') ? 'active' : ''}`}>
              Panel Mi Feria
            </Link>
          </li>
        )}
        ...
        {(user?.role === 'DRIVER' || user?.role === 'Repartidor') && (
          <li>
            <Link to="/driver" className={`pn-navbar-link ${location.pathname.startsWith('/driver') ? 'active' : ''}`}>
              Panel Delivery
            </Link>
          </li>
        )}
        ...
        <li>
          {user ? (
            <button className="pn-navbar-icon-btn" onClick={handleLogout} title="Cerrar sesión" aria-label="Cerrar sesión">
