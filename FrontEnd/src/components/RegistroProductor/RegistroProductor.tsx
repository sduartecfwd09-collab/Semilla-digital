...
const RegistroProductor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Solo clientes, usuarios o productores pueden acceder a esta página
    if (cachedUser.role !== 'Cliente' && cachedUser.role !== 'Usuario' && cachedUser.role !== 'Productor') {
      navigate('/perfil');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: role === 'Productor' ? 'Información actualizada.' : 'Tu solicitud para ser Productor ha sido enviada y será revisada por un administrador.',
        confirmButtonColor: 'var(--verde-claro)',
      }).then(() => {
        navigate(role === 'Productor' ? '/productor' : '/perfil');
      });
        ...
        <div className="registro-productor-card animate-fade">
          <div className="profile-header">
            <div className="header-info">
              ...
              <p>{role === 'Productor' ? 'Mantené actualizados los datos de tu puesto y feria.' : 'Completá los datos de tu puesto para que un administrador pueda revisar tu solicitud.'}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="registro-productor-form">
            ...
            <div className="registro-productor-actions">
              ...
              >
                {submitting
                  ? (role === 'Productor' ? 'Guardando...' : 'Enviando...')
                  : role === 'Productor'
                    ? 'Guardar cambios'
                    : solicitudEnviada
                      ? 'Actualizar solicitud'
                      : 'Enviar solicitud de Productor'
                }
              </button>
              ...
              {role === 'Productor' && (
                <button type="button" className="cancel-btn" onClick={() => navigate('/productor')}>
                  Volver al Dashboard
                </button>
              )}
            </div>
