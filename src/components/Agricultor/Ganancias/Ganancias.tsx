...
const Ganancias: React.FC = () => {
  ...
  const [costos, setCostos] = useState<CostosData>(() => {
    try {
      return JSON.parse(localStorage.getItem(`costos_${user?.id}`) || '{}')
    } catch { return {} }
  })
  ...
  // Resumen
  const totalGananciaEstimada = chartData.reduce((sum, d) => sum + d.ganancia, 0)
  const promedioMargen = chartData.length > 0 
        ...
        <div className="admin-main">
          ...
          <div className="admin-content">
            {loading ? (
              ...
              <p>No tienes productos registrados con precios. Agrega productos primero.</p>
            ) : (
              <div className="ganancias-page">
                {/* Stats */}
                <div className="ganancias-stats-grid">
                  <div className="ganancias-stat-card">
                    <div className="ganancias-stat-title">Productos Analizados</div>
                    <div className="ganancias-stat-value">{productosData.length}</div>
                  </div>
                  <div className="ganancias-stat-card">
                    <div className="ganancias-stat-title">Margen Promedio</div>
                    <div className={`ganancias-stat-value ${promedioMargen >= 40 ? 'success' : promedioMargen >= 20 ? 'warning' : ''}`}>
                      {promedioMargen.toFixed(1)}%
                    </div>
                  </div>
                  <div className="ganancias-stat-card">
                    <div className="ganancias-stat-title">Producto más rentable</div>
                    <div className="ganancias-stat-value success" style={{ fontSize: '1.4rem', marginTop: '0.4rem' }}>
                ...
                <div className="ganancias-section">
                  <div className="ganancias-section-header">
                    <h2 className="ganancias-section-title">📊 Top Ganancias por Unidad</h2>
                  </div>
                    ...
                    {chartData.map(d => {
                      ...
                      return (
                        <div className="chart-bar-group" key={d.id}>
                          ...
                          <div className="chart-label" title={d.nombre}>{d.emoji}</div>
                        </div>
                      )
                    })}
                  </div>
                ...
                <div className="ganancias-section">
                  <div className="ganancias-section-header">
                    <h2 className="ganancias-section-title">💵 Calculadora de Márgenes</h2>
                    <p style={{ fontSize: '0.85rem', color: '#64748b' }}>Ingresá tu costo estimado para calcular el margen.</p>
                  </div>
                          ...
                          <tr key={d.id}>
                            <td>
                              <div className="product-cell">
                                <span className="product-cell-emoji">{d.emoji}</span>
                                <span className="product-cell-name">{d.nombre}</span>
                              </div>
                            </td>
                            <td>₡{d.precioVenta.toLocaleString()} / {d.unidad || 'U'}</td>
                            <td>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                                ...
                                <input
                                  ...
                                  onChange={e => handleCostoChange(String(d.id), e.target.value)}
                                  placeholder="Ej: 500"
                                  min="0"
                                />
                              </div>
                            </td>
                            <td style={{ fontWeight: 700, color: d.ganancia > 0 ? '#052e16' : '#94a3b8' }}>
