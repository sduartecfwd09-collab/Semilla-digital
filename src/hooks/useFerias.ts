          ...
          for (const feria of newFerias) {
            try {
              await fetch(ENDPOINTS.ferias, {
                ...
                body: JSON.stringify({
                  // Adaptamos al formato de db.json
                  name: (feria as any).nombre,
                  province: (feria as any).provincia,
                  location: (feria as any).direccion,
                  schedule: `${(feria as any).dias}, ${(feria as any).horario}`,
                }),
              });
